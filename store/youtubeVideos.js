...
export const actions = {
  ...
  async showMore({commit, dispatch, state, getters}) {
    // if (this.pageNum > 0) {
    console.log("total:", state.totalResults, " maxRes:", state.maxResults, " page:", state.pageNum);
    const _hasMore = getters.hasMore;
    if (!_hasMore || state.nextPageToken === "") {
      console.log("here is last page", _hasMore, state.nextPageToken);
      return;
    }
    const fetchedObj = await dispatch("fetchVideos", state.searchText).catch(e => console.log(e));
  ...
  async fetchVideos({commit, dispatch, state, getters}, searchText = "") {
    ...
    try {
      return await this.$axios.$get(FROM.MOCK,
        {
          params: {
...
// async addVideo(vuexContext, post) {
//   const createdVideo = {
//     ...post,
//     updatedDate: new Date(),
//   };
//   const res = await this.$axios.$post("/posts.json", createdVideo)
//     .then(data => {
//       vuexContext.commit("addVideo", {
//         ...createdVideo,
//       });
//     })
//     .catch(e => {
...
//
// },
// editVideo(vuexContext, editedVideo) {
//   return this.$axios.$put(process.env.FIREBASE_DATABASE_URL + "/posts/"
//     + editedVideo.id
//     + ".json", editedVideo)
//     .then(() => {
//       vuexContext.commit("editVideo", editedVideo);
//     })
//     .catch(e => {
//       console.log(e);
//       return {
...
//
  setVideos({commit, dispatch}, fetchedVideosObj) {
    ...
    try {
      ...
      videoObj = _filter(fetchedVideosObj.items, x => x.id.videoId !== "")
        .map(y => ({
          etag: y.etag,
